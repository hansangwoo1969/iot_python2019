# 공공데이터 포털 / 대기오염정보 조회 서비스 / 대기질 예보 통보 조회 ____ json다운 안됨
...
def get_Weather_URL(yymmdd): # (1) 기상정보(동네예보정보 조회 서비스) request 보내기전, url만드는 함수
    end_point = "http://openapi.airkorea.or.kr/openapi/services/rest/ArpltnInforInqireSvc/getMinuDustFrcstDspth " # ForecastTimeData서비스를 이요할 것

    parameters = "?_returnType=json&ServiceKey="+access_key  # 매뉴얼을 보고 요청 메세지를 완성할 것
    parameters += "&searchDate=" + yymmdd
    parameters += "&InformCode="+ informcode
    # parameters += "&ver=1.1"
...
def Make_Weather_Json(yymmdd):  # (1) 기상정보 (동네예보정보 조회 서비스) json파일 생성하는 함수
    jsonData = get_Weather_URL(yymmdd)
    # print(jsonData)
...
informcode = 'PM10'      # PM10:미세먼지, PM25:초미세먼지, O3:오존
